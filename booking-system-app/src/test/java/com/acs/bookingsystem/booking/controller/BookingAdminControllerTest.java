...
class BookingAdminControllerTest {
  ...
  void givenNoAuth_whenAccessAdminEndpoint_thenReturns403() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/admin/bookings/{bookingUid}", BOOKING_UID))
        .andExpect(status().isForbidden());
  }

  @Test
  void givenAdmin_whenGetBookingByUid_thenReturns200() throws Exception {
    when(bookingService.getBookingByUid(BOOKING_UID)).thenReturn(bookingWithStatus);
    when(viewFactory.createView(any(BookingWithStatus.class), eq(Role.ROLE_ADMIN)))
  ...
  void givenAdmin_whenGetBookingsByUserUid_thenReturns200() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/admin/bookings/user/{userUid}", USER_UID))
        .andExpect(status().isOk())
  ...
  void givenAdmin_whenCancelBooking_thenReturns204() throws Exception {
    ...
    verify(bookingService).cancelBooking(eq(BOOKING_UID), any(User.class));
  }
}
